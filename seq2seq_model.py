...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        dk = hidden[0][0].size(-1)

        embedded = self.embedding(input).view(1, 1, -1)
        embedded = self.dropout(embedded)

        scores = torch.matmul(encoder_outputs,hidden[0].transpose(-2,-1)) / math.sqrt(dk)

        p_attn = F.softmax(scores, dim = -1)

        output = torch.matmul(p_attn.transpose(-1,0),encoder_outputs)

        output = torch.reshape(output,(1,1,512))
        ...
        return output, hidden, p_attn.transpose(-1,0)
...
def indexesFromSentence(lang, sentence):
    return [lang.word2index[word] if word in lang.word2index.keys() else lang.word2index['unknown'] for word in sentence.split(' ')]
...
if __name__ == '__main__':
    ...
    if usage == 'train':
        ...
        perplexity, encoder1, attn_decoder1 = run_train(iterations=1500) # 500 samples perplexity: 4.58290114593
        ...
        # to test a chatbot
        encoder1 = torch.load('model/1-layer/encoder.pkl',map_location='cpu')
        attn_decoder1 = torch.load('model/1-layer/decoder.pkl',map_location='cpu')
        input_sentence = ''
